...
def get_datasets(dataset, path, num_tasks, nc_first_task, validation, trn_transform, tst_transform, class_order=None):
    ...
    if  dataset == 'mnist':
    ...
    elif dataset == 'cifar100' or dataset == 'cifar100_icarl':
        ...
        # Dataset from "http://cs231n.stanford.edu/tiny-imagenet-200.zip",
        # 200 classes each image size (64,64)
        if not os.path.exists(os.path.join(path, 'train.txt')) or not os.path.exists(os.path.join(path, 'test.txt')):
            # Generate train.txt and text.txt from original dataset
            train = []
            test = []
            class_ids = np.loadtxt(os.path.join(path, 'wnids.txt'), dtype=str)
            train_path = os.path.join(path, 'train')
            for class_dir in os.listdir(train_path):
                images_dir = os.path.join(train_path, class_dir + '/images')
                for image_name in os.listdir(images_dir):
                    train.append([os.path.join('train/' + class_dir + '/images' , image_name), str(np.where(class_ids==class_dir)[0][0])])

            # As we don't have labels for test set, we use validation set as our test set.
            val_annotations = np.loadtxt(os.path.join(path, 'val/val_annotations.txt'), dtype=str)
            for line in val_annotations:
                image_name = line[0]
                class_code = line[1]
                test.append([os.path.join('val/images/' + image_name), str(np.where(class_ids==class_code)[0][0])])

            np.savetxt(os.path.join(path, 'train.txt'), train, fmt="%s")
            np.savetxt(os.path.join(path, 'test.txt'), test, fmt="%s")            
        ...
        # Dataset from "https://www.kaggle.com/datasets/greatgamedota/vggface2-test?select=n000106",
        # 500 classes each image size has different size
        if not os.path.exists(os.path.join(path, 'train.txt')) or not os.path.exists(os.path.join(path, 'test.txt')):
            # Generate train.txt and text.txt from original dataset
            train = []
            test = []
            for class_id, class_dir in enumerate(os.listdir(path)):
                images_name = os.listdir(os.path.join(path, class_dir))
                num_train = int(0.8*len(images_name))
                for i, name in enumerate(images_name):
                    if i < num_train:
                        train.append([os.path.join(class_dir, name), str(class_id)])
                    else:
                        test.append([os.path.join(class_dir, name), str(class_id)])
        ...
        # Dataset from http://www.vision.caltech.edu/datasets/cub_200_2011/
        # 200 Classes and each image has different size
        if not os.path.exists(os.path.join(path, 'train.txt')) or not os.path.exists(os.path.join(path, 'test.txt')):
            # Generate train.txt and text.txt from original dataset
            images = np.loadtxt(os.path.join(path, 'images.txt'), dtype=str)
            image_class_labels = np.loadtxt(os.path.join(path, 'image_class_labels.txt'), dtype=str)
            train_test_split = np.loadtxt(os.path.join(path, 'train_test_split.txt'), dtype=str)
            train = []
            test = []
            for (id1, img_path), (id2, class_idx), (id3, istrain) in zip(images, image_class_labels, train_test_split):
                assert id1 == id2 == id3
                # CUB200 starts counting classes from 1 to 200
                if istrain == '1': 
                    train.append(['images/'+img_path, str(int(class_idx)-1)])
                else:
                    test.append(['images/'+img_path, str(int(class_idx)-1)])
            print(len(train))
            print(len(test))
            np.savetxt(os.path.join(path, 'train.txt'), train, fmt="%s")
            np.savetxt(os.path.join(path, 'test.txt'), test, fmt="%s")            
