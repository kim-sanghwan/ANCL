...
class Appr(Inc_Learning_Appr):
    ...
    def __init__(self, model, device, nepochs=100, lr=0.05, lr_min=1e-4, lr_factor=3, lr_patience=5, clipgrad=10000,
                 ...
                 logger=None, exemplars_dataset=None, lamb=100, alpha=0.5, fi_num_samples=-1):
    ...
    def extra_parser(args):
        ...
        parser.add_argument('--lamb', default=100, type=float, required=False,
    ...
    def estimate_parameter_importance(self, trn_loader):
        ...
        importance = {n: torch.zeros(p.shape).to(self.device) for n, p in self.model.model.named_parameters()
                      if p.requires_grad}
        # Compute importance for specified number of samples -- rounded to the batch size
